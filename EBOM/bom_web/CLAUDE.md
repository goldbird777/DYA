...
- **2026-08-02: 카티아 자동 변환 · 웹 뷰어 · 고객EO 게시판 · 도면관리 범주 분리.**
  - **★CATPart → STEP 변환은 «CATIA 가 있는 윈도우 PC»에서만 가능하다.** 서버(리눅스)는 못 한다.
    다쏘 전용 비공개 형식이라 오픈소스 변환기가 없고 CATIA 는 라이선스가 필요하다.
    → `tools/catia_convert_agent.py`(사내 PC 실행). 서버는 «변환 대기 목록»만 제공하고
    변환·업로드는 그 PC 가 한다. 사용자 입장에선 자동(올려 두면 채워짐).
    **실측(사용자 PC, CATIA V5-6R2024 R34)**: `GetActiveObject('CATIA.Application')` 연결 성공,
    CATPart 47MB→STEP 64.6MB(35초), CATDrawing 55MB→PDF 4.5MB(25초).
    · **도면 PDF 는 «시트마다 따로» 나온다**(시트_1 4.44MB / DAYOU 0.08 / REVISION 0.00 …).
      **가장 큰 것만 본 도면**이고 나머지는 양식·리비전·디테일 → 버린다(사용자 확정).
      처음엔 합본을 만들었다가 불필요한 쪽이 섞여 방식을 바꿨다.
    · **Visible 을 건드리지 않는다** — 사람이 쓰던 CATIA 창을 숨기면 안 된다.
    · 인증은 사람 계정이 아니라 `X-Agent-Key`(app_meta `convert_agent_key`) — 무인 실행이라.
  - **웹 뷰어(`/catia/viewer/{id}`)**: Online3DViewer(MIT) + occt-import-js(OpenCASCADE WASM)
    **8.5MB 자체 호스팅**(`static/viewer/`). 외부 CDN 안 씀 — 사내망에서 막히면 못 열린다.
    **★실측으로 잡은 함정 2개**: ①o3dv 는 «URL 끝 확장자»로 형식을 판별하는데 우리 다운로드
    주소엔 확장자가 없어 아무것도 안 열렸다 → 파일을 받아 «원래 파일명을 붙인 File」로 넘긴다.
    ②o3dv 는 occt 워커를 **blob URL Worker** 로 만드는데 blob 워커 안에서는 `/static/...` 같은
    **루트 상대경로가 무효**다 → `location.origin` 을 붙여 절대 URL 로 배포본을 패치했다.
    STEP 12MB 가 1초에 렌더링됨.
  - **도면 표 배치(사용자 캡처안)**: «원본 받기 | 변환 O/X | 웹뷰어» 세 칸. 변환본이 없을 때도
    빨간 **X** 로 «없음»이 보여야 한다(«-» 로는 안 보인다는 지적). 2D 표는 «PDF 파일»,
    3D 표는 «STP 파일»로 머리글이 바뀐다. 정렬 기본은 **리비전 최신순 → 2D 먼저**(같은 리비전의
    도면·형상이 나란히), 열 제목 클릭으로 탐색기식 정렬.
    **★업로드 중복검사에 «확장자»를 넣어야 한다** — 안 넣으면 변환본이 중복으로 걸러진다.
  - **메뉴 재편**: «도면 관리»(품목·카티아), «EO 시스템»(설계변경 통보서·고객EO 등록),
    고객 마스터는 «마스터 데이터»로(전사 공통 키라 ERP·SCM 과 코드가 맞아야 한다는 사용자 판단).
  - **고객 EO 게시판(`/cust-eo`)**: 사내 PLM「고객EO 등록」과 동일 구조. 고객EO ↔ 내부 통보서
    다대다 상호 하이퍼링크(같은 표를 양방향 사용), 유사 고객EO 는 **대칭 저장**.
    **도면현황은 파일을 다시 안 올린다** — «고객EO → 연결된 통보서 → 품목현황 품번 →
    카티아 2D/3D» 로 따라가고, **배포용은 «배포완료» 상태일 때만** 내려받게 한다.
    결재는 내부 EO 로직 재사용 + `eo_approvals.doc_type` 으로 구분(간섭 없음 실측 확인).
  - **★템플릿을 다른 화면의 head 로 조립할 때 CSS 누락 주의** — cust_eo.html 에 모달 CSS 가
    빠져 창이 오버레이가 아니라 페이지에 붙어 «X 눌러도 안 닫히는» 버그가 났다.
    이후 모든 모달은 머리말·바닥글 sticky + Esc 닫기를 넣었다.
  - **미결**: ①협력사 도면 배포 게시판(협력사 로그인 방식으로 확정, 조직도에 협력업체 150곳
    이미 등록). 배포 단위는 EO 기준 + 품번 직접지정. **협력사에 원본도 주는지 STP 만 주는지
    사용자 확인 필요.** ②`rototo`(배승현) 계정 아이디를 PLM 쪽(`bsh0719`/`seunghyeon_bae`)에
    맞춰야 조직도 연결됨.

- **2026-08-02: 메뉴 범주 재편 + 카티아↔품목 연동 + 본문 전용 확대.** 커밋 `0022a44`.
  - **범주 재편**: E-BOM 시스템에 게시판이 몰려 헷갈린다는 지적 → **«도면 관리»**(품목 관리 +
    카티아 2D/3D)와 **«EO 시스템»**(설계변경 통보서) 신설. EO 시스템은 **고객 EO 배포 게시판이
    들어올 자리**로 미리 뺀 것. 카티아 게시판 **좌우(LH/RH) 필터 삭제** — 차종>열>부품군으로
    좁힌 뒤엔 좌우가 섞여 다 보이는 게 덜 헷갈린다는 사용자 판단(LR 값은 목록 열에 유지).
  - **★X 접두 = 양산 전 품번 (사용자 확정, 2026-08-02)**: `X88010-P1010`(개발/시작차)과
    `88010-P1010`(양산)은 **같은 제품으로 취급한다.** 요새는 양산 시작하며 바로 양산 금형으로
    가지만 때때로 X를 붙이기도 한다. → `base_part_no()`가 선행 X를 떼고 대조. 검색은 양방향,
    X품번 등록 시 양산 품번이 이미 있으면 그쪽에 붙이고 중복 생성 안 함.
  - **카티아↔품목 매칭이 실측 32품번 중 0개**였던 원인 두 가지: ①**카티아 업로드가 품목
    마스터에 등록을 안 하고 있었다**(BOM 업로드만 자동등록 — 기능 누락) ②위 X 접두 문제.
    둘 다 수정. **BOM 이 먼저든 도면이 먼저든 순서를 안 타게** 양쪽에서 등록한다.
    빈 칸만 채워 수기 스펙 보존. 기존 업로드분은 `backfill_parts_from_catia()`로 반영
    (서버 실행 완료 — 품목 10건 추가, 재업로드 불필요).
  - **품목관리에서 2D/3D를 «업로드»하게 바꾸지 않았다** — 도면은 카티아 게시판이 관리하고
    품목관리는 **보여주기만** 한다(사용자 판단이 맞음). 품목 목록에 `2D`·`3D` 열(최신 리비전
    배지), 상세에 «카티아 2D/3D» 카드(리비전·EO·일자·다운로드).
  - **본문 전용 확대**: Ctrl+휠은 헤더·사이드바까지 커지는 게 **브라우저 표준 동작이라 CSS로
    못 막는다**. 폰트 고정은 시력 나쁠 때 오히려 불편해져서 비권장. → `_sidebar.html`에
    «본문 크기» 조절(80~200%, 8단계) 추가. `.content{zoom:var(--content-zoom)}` — `zoom`은
    `transform`과 달리 **스크롤·클릭 좌표가 안 어긋난다**. localStorage 저장.
    실측: 125%에서 사이드바 폭 220px 불변.
  - **★배포 함정: nginx `client_max_body_size`** — 카티아 업로드가 «응답을 읽지 못했습니다»로
    실패한 진짜 원인은 앱이 아니라 nginx의 **50M 제한**이었다(에러 로그: `client intended to
    send too large body`). 카티아 3D는 한 개가 50~120MB라 **파일 하나도 못 올라갔다.**
    → `2G`로 상향 + `proxy_request_buffering off`(nginx가 통째로 버퍼링하지 않고 앱으로 흘려보냄).
    설정은 `/etc/nginx/sites-available/dya-bom`, 백업 `.bak.260801`. 실측 200MB 통과 8.1초.
    **큰 파일을 다루는 기능을 만들면 nginx 제한부터 확인할 것.**
  - 업로드는 **2개/80MB 묶음**으로 잘라 순차 전송(진행률·남은시간 표시, 실패 시 그 묶음만 재시도).
    **폴더째 올리기**(webkitdirectory) 지원 — 하위 폴더·OLD·TEMP 포함, Thumbs.db 자동 제외.
  - **미결**: ①`EO.X88391-...`·`..._R01`·`..._CATPART` 처럼 규칙에서 벗어난 파일명이 있다.
    건수가 많으면 파서를 넓힐 것(현재는 «미인식»으로 두고 화면에서 수기 보정). ②조직도 명단
    등록 후 `rototo`(배승현)는 PLM 아이디가 `bsh0719`/`seunghyeon_bae`라 계정 아이디 정정 필요.
    ③고객 EO 배포 게시판. ④CATIA 뷰어 오픈소스 검토(계속 유예).
