...
- **2026-08-01: 카티아 2D/3D 관리 게시판(`/catia`) 신설 — 폴더 트리를 «필터 칩»으로 대체.**
  커밋 `987235a`. R: NAS 실측(SP3/기본차/양금시작차/FRT/03_PLASTIC, 598파일 17GB)이 근거.
  - **파일명이 규칙이다**: `품번__리비전__품명__EO번호__날짜.확장자` — 실측 **일치율 98.8%**.
    올리면 메타를 자동 추출하므로 사용자는 **차종·열·부품군·단계 4개만** 고른다.
    NQ5도 동일 규칙(`88010-P1510__A__…__NQ5PE-EO-23-001__231026`). 품번 형식이
    품목 마스터(`88005-P1000`)와 같아 **매칭이 그대로 붙는다**.
  - **소분류 폴더를 더 파지 않기로 확정**(사용자 질문 "플라스틱도 쉴드커버·리클레버로 나눠야 하나").
    실측 반증 3가지: ①`03_PLASTIC` 안에 **PAD 11품번**이 섞여 있음 ②`01_2D` 폴더에 **3D 파일 5개**
    ③부품군마다 폴더 깊이가 제각각(PLASTIC 평면·PAD 2단·협력사 승인품은 «공급사»축).
    대신 **품명 첫 토큰이 곧 유형**(실측 100품번→27종: 쉴드커버15·백보드12·패드11·레버8…)이라
    자동 추출해 칩으로 준다. **폴더는 하나 고르면 나머지가 사라지지만 칩은 조합된다.**
  - **목록은 파일이 아니라 «품번 1행»** — 598파일→100행. 펼치면 리비전 이력. 파일을 다 늘어놓으면
    그게 곧 탐색기라 웹으로 옮긴 의미가 없다(사용자 지적: 웹은 화면이 작다).
  - **폴더에선 안 보이던 이상 자동 검출**(SP3 플라스틱 실측 **17건**): 리비전 중복(`88010-BS510`에
    K 두 개·L 결번), 결번, 2D/3D 한쪽 누락(3D만 63품번 vs 양쪽 36), 같은 품번·리비전인데 **품명이
    두 가지**(`88014-BS010__A__BRKT LEVER-FR…` vs `BRKT-FR…LEVER`), 품목마스터 품명 불일치.
  - **용량이 핵심 제약**: 03_PLASTIC 하나가 17GB, 같은 조건 전 부품군 **190GB**(3D 프레임만 113GB),
    차종20×단계4×열2면 수 TB. **서버 여유는 40GB**. 지금은 사용자 요청대로 «업로드형»으로 만들었고,
    확대하려면 **NAS 경로 색인형**(메타만 DB + UNC 경로 복사 + 사내 PC 스캐너)으로 가야 한다.
  - 업로드는 **1MB 청크 스트리밍**(카티아 원본 수십~수백 MB, 통째로 읽으면 `MemoryMax=750M`에 걸림).
    실측 NQ5 10파일 1.83GB 업로드 정상, 다운로드 **MD5 원본 일치**.
  - `part_type`/`side`/`rev_sort`는 파생값이라 **`CATIA_DERIVE_VER`로 재계산**(`refresh_catia_derived`,
    `app_meta` 테이블). 추출 규칙을 고쳐도 재업로드 불필요 — 수기 보정한 품번·리비전은 안 건드린다.
  - **리비전 표기는 아직 확정 안 함**(사용자: 나중에 최적화). 현실은 `00→A→…→R`이 주류인데
    `__R01` 표기와 `XA/XB/XC`(시작차)도 섞여 있다. **파일명에 있는 걸 그대로 저장하고 정렬만** 담당
    (`_rev_sort`: 숫자<알파벳<R01형). 규칙이 정해지면 이 함수만 고치고 VER 올리면 된다.
  - 부품군 = 프레임·패드·플라스틱·헤드레스트·**암레스트**·협력사 승인품·사급품·HW·기타.

- **2026-08-01: 설계변경 통보서 — 저장 즉시 결재 가능 + 결재자 선택 창.** 커밋 `6ee8e6c`.
  «EO 번호를 먼저 저장하면 결재 버튼이 눌리게» 요청 반영. 저장 후 **서버가 돌려준 id로 그 건을
  바로 연다** — 기존엔 목록 첫 줄로 짐작해서 정렬에 따라 엉뚱한 건이 열릴 수 있었다.
  결재란 «작성» 칸은 로그인 계정 성명으로 자동·읽기전용(기안자), 검토·승인 칸은 «👤 선택» 버튼으로
  계정 목록(부서별 그룹+검색)에서 고른다(`/eo/users`). 계정에 등록된 사람만 골라야 본인 화면에
  승인 버튼이 뜨기 때문. 서버 사용자 16명 전원 성명 입력돼 있어 바로 쓸 수 있다.
  - **미결**: 사내 PLM(`ds.dayou.co.kr`, 222.119.214.68) **조직도 연동** — 오라클 서버에서
    **접속 가능함을 확인**(HTTP 302). 다만 자동 로그인은 자격증명 취급 문제가 있어, 조직도 1회
    내보내기→마스터 등록을 권장. 사용자 크롬 화면 분석은 사용자가 해당 화면을 열어 줘야 진행.
